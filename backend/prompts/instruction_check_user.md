...
### 5. ИЗМЕНЕНИЕ В ТАБЛИЦАХ:
- Изменение строки в таблице: "в таблице X строку Y изложить в редакции", "в таблице X заменить Y на Z"
- Изменение ячейки таблицы: "в таблице X в строке Y заменить Z"
- ВАЖНО: Для изменений в таблицах используй REPLACE_TEXT:
  - target.text: точное содержимое ячейки или строки для поиска
  - payload.new_text: новое содержимое
  - target.match_case: false (для лучшего поиска)
- Примеры:
  - {"operation": "REPLACE_TEXT", "target": {"text": "ДРМ", "match_case": false}, "payload": {"new_text": "ДКР Департамент кредитных рисков"}}
  - {"operation": "REPLACE_TEXT", "target": {"text": "ПД", "match_case": false}, "payload": {"new_text": "ПД Проектные дирекции 1,2,3,4,5,6."}}
...
- Если упоминается "в таблице X строку Y", используй REPLACE_TEXT с точным содержимым строки
- Для "изложить в редакции" в контексте таблицы - это замена содержимого ячейки
- Игнорируй название таблицы в target.text, используй только содержимое ячейки
- Для аббревиатур в таблицах используй только саму аббревиатуру как target.text
- Примеры правильного распознавания:
  ...
  * "по всему тексту ДРМ заменить" → target.text: "ДРМ", target.replace_all: true
