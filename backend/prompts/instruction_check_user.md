...
- Общий принцип: игнорируй название таблицы, используй только содержимое ячейки как target.text
...
- Для коротких значений (аббревиатуры, коды) используй только само значение как target.text
- Общие примеры распознавания:
  * "в таблице строку [значение] изложить" → target.text: "[значение]"
  * "по всему тексту [старое] заменить на [новое]" → target.text: "[старое]", target.replace_all: true
