...
"""
    ...
    async def _parse_changes_with_llm(
        ...
        action_count = sum(1 for keyword in action_keywords if keyword.lower() in changes_text.lower())
        ...
        logger.info(f"Найдено паттернов инструкций: {pattern_count}, ключевых слов действий: {action_count}, оценка количества инструкций: {estimated_instructions}")
        logger.info(f"LLM распознал изменений: {len(changes)}")
        
        # Если оценка количества инструкций больше найденных, предупреждаем
        if estimated_instructions > len(changes):
            logger.warning(
                f"⚠ ВНИМАНИЕ: В тексте, вероятно, содержится {estimated_instructions} инструкций "
                ...
                f"Рекомендуется проверить документ вручную и убедиться, что все инструкции включены."
            )
            ...
            try:
                ...
                if mcp_replaced:
                    ...
                    try:
                        ...
                        # Если верификация не прошла, пробуем локальную замену через MCP клиент (которая использует улучшенную логику)
                        if not verify_success:
                            logger.info(f"🔄 MCP вернул успех, но верификация не прошла, пробуем локальную замену через улучшенный метод")
                            # Используем метод _replace_text_locally_with_tables из MCP клиента
                            try:
                                ...
                                if local_replaced:
                                    ...
                                    if verify_success_after:
                                        replaced = True
                                        logger.info(f"✅ Локальная замена выполнена успешно и подтверждена после неудачной верификации MCP")
                                    else:
                                ...
                                logger.error(f"❌ Ошибка при локальной замене после верификации: {local_e}", exc_info=True)
