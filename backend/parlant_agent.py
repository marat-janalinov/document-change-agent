...
"""
    ...
    async def process_documents(
        ...
        try:
            ...
            # КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: НЕ создаем новый Document() здесь!
            # Это перезапишет все изменения, так как Document() читает файл с диска
            # Файл уже сохранен после каждого изменения в _handle_replace_text через doc.save(filename)
            # Просто проверяем, что файл существует и содержит изменения
            try:
                ...
                if os.path.exists(source_file):
                    ...
                    # Проверяем, что изменения действительно присутствуют в файле
                    try:
                        verify_doc = Document(source_file)
