...
"""
    ...
    async def _handle_replace_text(self, filename: str, change: Dict[str, Any], changes_file: Optional[str] = None, master_doc: Optional[Document] = None) -> Dict[str, Any]:
        ...
        # КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ: Используем логику локальной замены напрямую на master_doc
        ...
        # КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Если используется master_doc, НЕ вызываем локальную замену или MCP
        # которые сохраняют файл - все изменения должны быть только в master_doc
        # Если master_doc не используется (fallback), пробуем локальную замену и MCP как финальные попытки
        if not replaced and master_doc is None:
            ...
            try:
                ...
                # ШАГ 2: Если локальная замена не сработала, пробуем MCP replace_text (только если master_doc не используется)
                if not replaced and master_doc is None:
...
Если пункт не найден или не удалось точно определить, верни location_type: "unknown" с confidence < 0.7."""
    ...
    def _replace_text_in_document_object(
        self,
        doc: Document,
        old_text: str,
        new_text: str,
        ...
        try:
            ...
            if paragraph_index is not None and paragraph_index >= 0 and paragraph_index < len(doc.paragraphs):
                ...
                logger.info(f"Локальная замена в памяти: попытка в параграфе {paragraph_index} (strict={strict_paragraph})")
            else:
                target_paragraphs = doc.paragraphs
                strict_paragraph = False  # Глобальная замена не может быть strict
            ...
            for para_idx, paragraph in enumerate(target_paragraphs):
                ...
                if strict_paragraph and not text_found:
                    ...
                    try:
                        # Получаем весь текст параграфа через все возможные способы
                        all_text_variants = [
                            paragraph.text,  # Стандартный способ
                            "".join([run.text for run in paragraph.runs]),  # Объединение runs
                            " ".join(paragraph.text.split()),  # Нормализация пробелов
                            " ".join("".join([run.text for run in paragraph.runs]).split()),  # Нормализация объединенных runs
                        ]
                        
                        # Пробуем найти в каждом варианте
                        for variant_idx, variant_text in enumerate(all_text_variants):
                            if old_text in variant_text:
                                logger.info(f"✅ Текст найден в варианте {variant_idx}, выполняем замену")
                                new_variant_text = variant_text.replace(old_text, new_text, 1)
                                paragraph.clear()
                                paragraph.add_run(new_variant_text)
                                replacements_made += 1
                            ...
                            for para_idx, cell_para in enumerate(cell.paragraphs):
                                if old_text in cell_para.text:
                                    try:
                                        ...
                                        replacements_made += 1
                                        logger.info(f"✅ Локальная замена в памяти: заменен в таблице {table_idx}, ячейка [{row_idx},{cell_idx}], параграф {para_idx}")
                                    except Exception as e:
                                        logger.warning(f"Не удалось заменить в ячейке таблицы: {e}")
            
            if replacements_made > 0:
                logger.info(f"✅ Локальная замена в памяти выполнена: {replacements_made} замен текста")
                return True
            else:
                logger.warning(f"⚠️ Локальная замена в памяти: текст '{old_text[:100]}...' не найден в документе для замены")
                return False
                
        except Exception as e:
            logger.error(f"❌ Ошибка при локальной замене текста в памяти: {e}", exc_info=True)
            return False
